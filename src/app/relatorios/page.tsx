...
import { BarChart2, TrendingUp, TrendingDown, ClipboardList } from 'lucide-react';
...
export default function Relatorios() {
  ...
  useEffect(() => {
    ...
    ]).then(([t, o]) => { setTransactions(t); setOrders(o); });
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Stat label="Receita Total" value={`R$ ${income.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`} icon={<TrendingUp size={18} />} color="green" />
        <Stat label="Despesas Total" value={`R$ ${expense.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`} icon={<TrendingDown size={18} />} color="red" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-5">
          ...
          {transactions.length === 0 && <p className="text-zinc-500 text-sm">Sem dados</p>}
        </div>
      </div>
    </div>
  );
